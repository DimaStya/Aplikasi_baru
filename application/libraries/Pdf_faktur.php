...
class Pdf_faktur extends FPDF{
	...
	function Header()
	{
        ...
        if($title['head']['kode_cv'] == 'Tanpa CV'){
			...
			$mou = $title['head']['no_mou'];
		}
		if($title['head']['kode_kerjasama'] == 'Tanpa Pengajuan'){
			$nama_kerjasama = $title['head']['nama_kerjasama'];
			$no_pengajuan = '-';
		}else if($title['head']['kode_kerjasama'] != 'Tanpa Pengajuan'){
			$nama_kerjasama = $title['head']['nama_kerjasama'];
			$no_pengajuan = $title['head']['no_pengajuan'];
		}
		$this->Image(APPPATH .'/libraries/mediatama_f.jpg',111,3,0);
		$this->Ln(17);
		$this->SetFont('Times','B',12);
		$this->Cell(0,0,'FAKTUR PESANAN',0,0,'C');
		$this->Ln(5);
		// header
		...
		$this->Cell($w[3],6,'No Faktur','',0,'L',false);
        $this->Cell($w[4],6,': '.$title['head']['no_faktur'],'',0,'L',false);
        $this->Ln();
        //baris 2
		$this->Cell($a,6,'Bapak/Ibu '. $yth,'',0,'L',false);
        $this->Cell($w[2],6,'','',0,'L',false);
		$this->Cell($w[3],6,'Tanggal Faktur','',0,'L',false);
        $this->Cell($w[4],6,': '.$title['head']['tgl_faktur'],'',0,'L',false);
        $this->Ln();
        //baris 3
		$this->Cell($a,6,$data_alamat,'',0,'L',false);
        $this->Cell($w[2],6,'','',0,'L',false);
		$this->Cell($w[3],6,'No SJ','',0,'L',false);
        $this->Cell($w[4],6,': '.$title['head']['no_suratjalan'],'',0,'L',false);
        $this->Ln();
        //baris 4
		$this->Cell($a,6,$tlp,'',0,'L',false);
        $this->Cell($w[2],6,'','',0,'L',false);
		$this->Cell($w[3],6,'Tanggal SJ','',0,'L',false);
        $this->Cell($w[4],6,': '.$title['head']['tgl_sj'],'',0,'L',false);
        $this->Ln();
        //baris 5
		$this->Cell($a,6,'','',0,'L',false);
        $this->Cell($w[2],6,'','',0,'L',false);
		$this->Cell($w[3],6,'Nama Kerjasama','',0,'L',false);
        $this->Cell($w[4],6,': '.$nama_kerjasama,'',0,'L',false);
        $this->Ln();
        //baris 6
		$this->Cell($w[0],6,'Nama CV/Cust','',0,'L',false);
        $this->Cell($w[1],6,': '.$title['head']['nama_cv'].'/'.$title['head']['nama_customer'],'',0,'L',false);
        $this->Cell($w[2],6,'','',0,'L',false);
		$this->Cell($w[3],6,'No MoU','',0,'L',false);
        $this->Cell($w[4],6,': '.$mou,'',0,'L',false);
        $this->Ln();
        //baris 7
		$this->Cell($w[0],6,'Nama Plgn','',0,'L',false);
        $this->Cell($w[1],6,': '.$title['head']['nama_penerima'],'',0,'L',false);
        $this->Cell($w[2],6,'','',0,'L',false);
		$this->Cell($w[3],6,'No Pengajuan','',0,'L',false);
        $this->Cell($w[4],6,': '.$no_pengajuan,'',0,'L',false);
        $this->Ln();
        //baris 8
		$this->Cell($w[0],6,'No Telp Plg','',0,'L',false);
        ...
        $this->Ln();
        //baris 9
		$this->Cell($w[0],6,'Dana','',0,'L',false);
        $this->Cell($w[1],6,': '.$title['head']['sumber_dana'],'',0,'L',false);
        $this->Cell($w[2],6,'','',0,'L',false);
		$this->Cell($w[3],6,'Koordinator','',0,'L',false);
        $this->Cell($w[4],6,': '.$title['head']['nama_kaper'],'',0,'L',false);
        $this->Ln();
        //baris 10
		$this->Cell($w[0],6,'Alamat Kirim','',0,'L',false);
        $this->Cell($w[1],6,': '.$title['head']['alamat_penerima'],'',0,'L',false);
        $this->Cell($w[2],6,'','',0,'L',false);
