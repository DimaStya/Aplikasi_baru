...
class Pdf_stokmini extends FPDF{
	//Page header
	...
	function Header()
	{
		...
		$this->Cell(25,4,': '.$title['head']['no_do_stokmini'],'',0,'L',false);
		$this->Ln(20);
		$this->SetLineWidth(1);
	...
	function Content(){
		...
		$this->Cell(0,0,'SURAT PESANAN STOK MINI',0,0,'C');
		$this->Ln(5);
		// header
        ...
        $this->Cell(35,6,': '.$title['head']['no_do_stokmini'],'',0,'L',false);
        $this->Cell(45,6,'','',0,'L',false);
		$this->Cell(40,6,'Tanggal','',0,'L',false);
		$tanggal = explode("-", $title['head']['tanggal']);
        $this->Cell(60,6,': '.$tanggal['2'].'/'.$tanggal['1'].'/'.$tanggal['0'],'',0,'L',false);
        $this->Ln();
        //baris 2
		$this->Cell(40,6,'Alamat Kirim','',0,'L',false);
        $this->Cell(35,6,': '.$title['head']['alamat_kirim'],'',0,'L',false);
        $this->Cell(45,6,'','',0,'L',false);
		$this->Cell(40,6,'Nama Kaper',0,'L',false);
        $this->Cell(60,6,': '.$title['head']['nama_kaper'],0);
        //baris 3
		$this->Cell(40,6,'Keterangan','',0,'L',false);
        $this->MultiCell(130,6,': '.$title['head']['keterangan'],0);
		$this->Ln(6);
		...
		foreach ($title['tabel'] as $tabel ) {
			...
			$this->Cell(20,6,$tabel['jumlah'],'B',0,'R',false);//jumlah beli
